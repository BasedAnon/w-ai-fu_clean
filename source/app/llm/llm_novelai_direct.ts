...
export class LargeLanguageModelNovelAIDirect implements LargeLanguageModel {
    #child_process: cproc.ChildProcess;
    #websocket: WebSocket = new WebSocket(null);
    #websocket_server: WebSocketServer;

    constructor() {
        this.#child_process = cproc.spawn(ENV.PYTHON_PATH, ["novel_llm_direct.py"], {
    ...
    async initialize(): Promise<void> {
        return new Promise((resolve) => {
            this.#websocket_server.on("connection", (socket) => {
                ...
                IO.debug("Loaded LargeLanguageModelNovelAIDirect.");
                resolve();
            });
        });
    }
