...
export class TextToSpeechNovelAIDirect implements TextToSpeech {
    ...
    constructor() {
        this.#child_process = cproc.spawn(ENV.PYTHON_PATH, ["novel_tts_direct.py"], {
    ...
    initialize(): Promise<void> {
        return new Promise((resolve) => {
            this.#websocket_server.on("connection", (socket) => {
                ...
                this.#websocket.send("");
                IO.debug("Loaded TextToSpeechNovelAIDirect.");
                resolve();
            });
        });
    }
